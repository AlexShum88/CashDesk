...
public class AuthorizationServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        System.out.println(req.getSession().getAttribute("senior") + "user role "+ SENIOR_CASHIER.name.equals(user.getRole()));
        if (req.getSession().getAttribute("senior")==null && SENIOR_CASHIER.name.equals(user.getRole())){
            System.out.println("authServ#doPost#in if seinor==null & user senior");
            req.getSession().setAttribute("senior", user);
            System.out.println(req.getSession().getAttribute("senior"));
